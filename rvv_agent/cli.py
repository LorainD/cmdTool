...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--config", default=None, help="Path to rvv_agent.toml (default: ./rvv_agent.toml)")
    ...
    p_mig = sub.add_parser("migrate", help="Run pipeline: search -> LLM analysis -> LLM generate -> (optional) exec")
    p_mig.add_argument("symbol", help="Target symbol/function name")
    p_mig.add_argument("--ffmpeg-root", default=None, help="FFmpeg root dir (default from config)")
    p_mig.add_argument("--apply", action="store_true", help="Apply generated files into FFmpeg workspace (default: no)")
    p_mig.add_argument("--exec", action="store_true", help="Run configure + build checkasm (may take long)")
    p_mig.add_argument("-j", "--jobs", type=int, default=0, help="make jobs (default: cpu_count)")
